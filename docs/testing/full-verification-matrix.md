...
| `POST /api/v1/database-validation-checks/{id}/execute` | verified with automated tests | Direct HTTP database-governance test covers safe runtime execution of structured read-only Postgres checks, including table existence, migration-marker completion, missing-target row-count assertion failure, runtime-only connection resolution, persisted execution evidence, connection health updates only for connection/session outcomes, and release posture changes derived from execution truth |
...
| `POST /api/v1/releases` | verified with automated tests | Direct HTTP route test covers persisted release-bundle creation with deterministic release analysis, including release/config/database/change-window policy decisions, and browser governance flow exercises the same path |
...
| `POST /api/v1/rollout-executions` | verified and hardened | browser, integration, smoke, governed release-link coverage, blocking rollout-execution policy decisions, release policy blocker enforcement, and runtime-contract validation now cover the documented create envelope |
...
| `GET /api/v1/policy-decisions` | verified and hardened | HTTP policy tests cover persisted decision reads for risk, rollout, rollout-execution, release, config, database, and change-window workflows, rollout-block evidence, release/config/database scoped filters, and runtime contract validation covers the documented list envelope |
...
| `policy-decision list` | verified with automated tests | dedicated CLI test now covers persisted policy-decision reads with project/policy/change/risk/plan/rollout/release/config/database filters and organization-scope headers |
...
| Architecture docs | reviewed and aligned this pass | Auth/session, integration, SCM, outbox, security, OpenAPI, and release-readiness wording now separates implemented behavior from missing enterprise IAM, provider proof, secret-vault, migration-runner, external-policy, and replay-platform capabilities |
| README / local dev docs | reviewed and aligned this pass | The README and runbooks point to OpenAPI as the complete API source, list the current CLI/governance surfaces, and document release-readiness, live-proof preflight/checklist generation, preserved-proof validation, artifact secret-safety scanning, and proof-class truth |
