...
| `#/deployments` | verified and hardened | Browser tests now cover route-local loading/error states, bundled page-state request usage, server-backed search submit, rollback-only filtering, and paginated summary visibility |
...
| `#/enterprise` | verified and hardened | Browser tests cover route-local loading/error states, bundled page-state request usage, enterprise identity-provider visibility, provider creation, public SSO entry points, durable outbox diagnostics, retry/requeue controls with route-local refresh, failure feedback, read-only org-member behavior, and webhook diagnostics visibility |
...
| Browser session persistence | verified with automated tests | In-memory auth tests cover issue, expiry, revocation, and logout invalidation; PostgreSQL round-trip test covers create, lookup-by-hash, and revoked-session persistence |
...
| Duplicate rollback prevention | partially verified | deterministic runtime checks and workflow tests; no multi-worker lease proof |
...
| Secret leakage prevention | verified with automated tests | `live-proof-verify` command tests assert saved proof reports do not embed configured GitHub/GitLab/Kubernetes/Prometheus secret values, and the new `artifact-safety-check` command tests plus `make release-readiness` now scan generated release reports, supporting logs, and preserved proof artifacts for configured secret-backed env values without printing those secret values back out. This is still not a universal runtime metadata scrubber. |
...
| `status list` | verified and hardened | command test now covers the server-backed search endpoint, richer filters, and output |
...
| `live-proof-verify` | verified and hardened | command tests cover hosted-like GitLab and GitHub App proof runs, repository/runtime mapping, explicit proof environment-class handling, fail-fast missing-secret validation, hosted-SaaS/local-endpoint rejection, secret-safe report generation, saved-report validation mode, and richer proof check/evidence output |
...
| README / local dev docs | reviewed and aligned this pass | release-readiness, preserved-proof validation, artifact secret-safety scanning, and proof-class truth are now documented directly in README and runbooks |
