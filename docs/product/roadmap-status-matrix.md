...
| Policy-as-code deployment governance | `partial` | Persisted deterministic policies and decisions exist for `risk_assessment`, `rollout_plan`, `rollout_execution`, `release_bundle`, `config_set`, `database_governance`, and `change_window`. Release analysis now carries the matched decisions plus plain-language policy posture, and release-linked rollout execution creation blocks only on actual blocking policy decisions or truly blocking database posture. DB-aware governance also contributes deterministic blockers/highlights for irreversible or insufficiently validated database changes before a rollout execution can be created from a linked release. | This remains a deterministic in-product rules layer, not OPA/Rego, a general external policy backend, or a full approval-workflow engine for every operational action. |
