...
func (s *HTTPServer) routes() {
	...
	s.mux.HandleFunc("PATCH /api/v1/config-sets/{id}", s.withAuth(s.updateConfigSet))
	s.mux.HandleFunc("GET /api/v1/releases", s.withAuth(s.listReleases))
	s.mux.HandleFunc("POST /api/v1/releases", s.withAuth(s.createRelease))
	s.mux.HandleFunc("GET /api/v1/releases/{id}", s.withAuth(s.getRelease))
	s.mux.HandleFunc("PATCH /api/v1/releases/{id}", s.withAuth(s.updateRelease))
	s.mux.HandleFunc("GET /api/v1/database-connection-references", s.withAuth(s.listDatabaseConnectionReferences))
	s.mux.HandleFunc("POST /api/v1/database-connection-references", s.withAuth(s.createDatabaseConnectionReference))
	s.mux.HandleFunc("GET /api/v1/database-connection-references/{id}", s.withAuth(s.getDatabaseConnectionReference))
	s.mux.HandleFunc("PATCH /api/v1/database-connection-references/{id}", s.withAuth(s.updateDatabaseConnectionReference))
	s.mux.HandleFunc("POST /api/v1/database-connection-references/{id}/test", s.withAuth(s.testDatabaseConnectionReference))
	s.mux.HandleFunc("GET /api/v1/database-connection-tests", s.withAuth(s.listDatabaseConnectionTests))
	s.mux.HandleFunc("GET /api/v1/database-connection-tests/{id}", s.withAuth(s.getDatabaseConnectionTest))
	s.mux.HandleFunc("GET /api/v1/database-changes", s.withAuth(s.listDatabaseChanges))
	s.mux.HandleFunc("POST /api/v1/database-changes", s.withAuth(s.createDatabaseChange))
	s.mux.HandleFunc("GET /api/v1/database-changes/{id}", s.withAuth(s.getDatabaseChange))
	s.mux.HandleFunc("PATCH /api/v1/database-changes/{id}", s.withAuth(s.updateDatabaseChange))
	s.mux.HandleFunc("GET /api/v1/database-validation-checks", s.withAuth(s.listDatabaseChecks))
	s.mux.HandleFunc("POST /api/v1/database-validation-checks", s.withAuth(s.createDatabaseCheck))
	s.mux.HandleFunc("GET /api/v1/database-validation-checks/{id}", s.withAuth(s.getDatabaseCheck))
	s.mux.HandleFunc("PATCH /api/v1/database-validation-checks/{id}", s.withAuth(s.updateDatabaseCheck))
	s.mux.HandleFunc("POST /api/v1/database-validation-checks/{id}/execute", s.withAuth(s.executeDatabaseCheck))
	s.mux.HandleFunc("GET /api/v1/database-validation-executions", s.withAuth(s.listDatabaseExecutions))
	s.mux.HandleFunc("GET /api/v1/database-validation-executions/{id}", s.withAuth(s.getDatabaseExecution))
